"""
Fig 1 — Forest Plot: LDCT + Traditional ML (k=68)
DerSimonian-Laird random-effects meta-analysis on logit-transformed AUC.
Studies sorted by AUC; bottom-15 and top-15 displayed when k>30.
Marker size proportional to RE weight; color by validation type.
Output: fig1_forest_LDCT_traditional.png
"""
...
OUT_PNG  = "../fig1_forest_LDCT_traditional.png"
...
# Display: bottom-15 + top-15 when k > 30
if len(sub) > 30:
    sub_show = pd.concat([sub.head(15), sub.tail(15)]).reset_index(drop=True)
    truncated = True
    n_hidden = len(sub) - 30
else:
...
w_norm = (w_show / w_show.max()) * 200 + 20   # marker area 20–220
...
# Truncation gap
if truncated:
    gap_y = (y_pos[14] + y_pos[15]) / 2
    ax.text(0.51, gap_y, f"  ... {n_hidden} studies not shown ...",
            ha="left", va="center", fontsize=7.5, color="#666666", style="italic")
    ax.axhline(gap_y, color="#cccccc", lw=0.5, ls="--", zorder=0)
...
ax.set_title("Forest Plot - LDCT + Traditional ML\n"
